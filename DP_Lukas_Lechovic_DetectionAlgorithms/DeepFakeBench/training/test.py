...
def test_epoch(model, test_data_loaders):
    ...
    for key in keys:
        ...
        main_metrics = ['acc', 'auc', 'eer', 'ap', 'video_auc']
        for k in main_metrics:
            if k in metric_one_dataset:
                v = metric_one_dataset[k]
        ...
        for metric_key, metric_label in additional_metrics:
            if metric_key in metric_one_dataset:
                v = metric_one_dataset[metric_key]
                if isinstance(v, (int, float, np.integer, np.floating)):
                    tqdm.write(f"  {metric_label}: {v:.4f}")
        tqdm.write("=" * 60)
