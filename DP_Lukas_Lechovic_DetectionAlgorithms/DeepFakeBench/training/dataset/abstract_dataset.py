...
class DeepfakeAbstractBaseDataset(data.Dataset):
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                def extract_number_from_filename(filepath):
                    ...
                    if '\\' in filepath:
                        ...
                        filename = filepath.split('/')[-1].split('.')[0]
                    # Extract numeric part (handles both '01255' and 'fake_01255')
                    import re
                    numbers = re.findall(r'\d+', filename)
                    return int(numbers[-1]) if numbers else 0

                frame_paths = sorted(frame_paths, key=extract_number_from_filename)
    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
            # Check if path is absolute (contains : for Windows or starts with / for Unix)
            if not (file_path[0] == '.' or ':' in file_path or file_path[0] == '/'):
        ...
        if not self.lmdb:
            ...
            with self.env.begin(write=False) as txn:
                ...
                if image_bin is None:
                    ...
                    image_buf = np.frombuffer(image_bin, dtype=np.uint8)
                    # cv2.IMREAD_GRAYSCALE为瘰度图，cv2.IMREAD_COLOR为彩色图
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                # Predchádzaj nekonečnej rekurzii - skús náhodný iný index
                print(f"Error loading image at index {index}: {e}")
                if index != 0:  # Ak nie je index 0, skús index 0
                    return self.__getitem__(0)
                else:
                    # Ak aj index 0 zlyhá, vráť dummy data
                    print("Returning dummy data due to loading errors")
                    dummy_image = torch.zeros(3, self.config['resolution'], self.config['resolution'])
                    dummy_label = 0
                    dummy_landmarks = None if not self.config['with_landmark'] else torch.zeros(81, 2)
                    dummy_mask = None if not self.config['with_mask'] else torch.zeros(self.config['resolution'], self.config['resolution'], 1)
                    return dummy_image, dummy_label, dummy_landmarks, dummy_mask
