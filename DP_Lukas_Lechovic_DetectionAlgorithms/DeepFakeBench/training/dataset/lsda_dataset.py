...
class LSDADataset(DeepfakeAbstractBaseDataset):
    ...
    on_3060 = torch.cuda.is_available() and "3060" in torch.cuda.get_device_name()
    transfer_dict = {
        'youtube':'FF-real',
        'Deepfakes':'FF-DF',
        'Face2Face':'FF-F2F',
    ...
    def __init__(self, config=None, mode='train', with_dataset=['Deepfakes', 'Face2Face', 'FaceSwap', 'NeuralTextures']):
        ...
        with open(self.data_list[mode], 'r') as fd:
            ...
            for pair in data:
                r1, r2 = pair
                step = 1
                # collect a group with 1+len(fakes) videos, each video has self.frames[mode] frames。这里就是按嘌一个video这种顺序来存的，所以读的时候自然只覘有了offset，就蘽对应的取了
                #此外，这里面存的压根就不是路径，而是规茘化的内容。
    ...
    def load_image(self, name, idx):
        ...
        #其实并没有完全对应，而只是保诘在嘌一video的目标时间区间内的一帧
    ...
    def __getitem__(self, index):
        name, idx, label, mode = self.img_lines[index] #这个sampler的目的是不覘取重复video的图。
        label = int(label)  # specific fake label from 1-4

        #取img没什乘好说的。然嘎在这里把规茘化的img_lines转为实际路径。
        try:
            img = self.load_image(name, idx)
        except Exception as e:
            # 下面处瘆不太嘘适，取的不是预期的video_id/fake_method，影响嘎面的lsda。
